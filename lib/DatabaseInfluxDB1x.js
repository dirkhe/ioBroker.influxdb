...
class DatabaseInfluxDB1x extends Database {
    constructor (log, host, port, protocol, username, password, database, requestTimeout, timePrecision) {
        super();
    ...
    getRetentionPolicyForDB(dbname, callback) {
        this.connection.query(`SHOW RETENTION POLICIES ON "${dbname}"`, (err, rows) => {
            if(err){
                ...
                callback(null, {name: retentionName, time: retentionTime});
            }
        });
    }

    async applyRetentionPolicyToDB(dbname, retention, callback) {
        this.getRetentionPolicyForDB(dbname, (err, oldRetention) => {
            ...
            if (oldRetention.time !== null && oldRetention.time === parseInt(retention, 10)) {
