...
const DatabaseInfluxDB1x =
  require("./lib/DatabaseInfluxDB1x.js").DatabaseInfluxDB1x; // TODO if else
const DatabaseInfluxDB2x =
  require("./lib/DatabaseInfluxDB2x.js").DatabaseInfluxDB2x;
const DatabaseInfluxDB3x =
  require("./lib/DatabaseInfluxDB3x.js").DatabaseInfluxDB3x;
    ...
    if (
      ...
      if (
        ...
        if (obj.common.custom[adapter.namespace].aliasId !== id) {
          ...
          adapter.log.debug(
            `Registered Alias: ${id} --> ${adapter._aliasMap[id]}`
          );
          id = adapter._aliasMap[id];
          checkForRemove = false;
        } else {
          adapter.log.warn(
            `Ignoring Alias-ID because identical to ID for ${id}`
          );
          obj.common.custom[adapter.namespace].aliasId = "";
        }
      }
      ...
      if (
        ...
        if (
          ...
          obj.common.custom[adapter.namespace].blockTime =
            parseInt(adapter.config.blockTime, 10) || 0;
        } else {
          obj.common.custom[adapter.namespace].blockTime =
      ...
      if (
        ...
        obj.common.custom[adapter.namespace].round = parseInt(
          obj.common.custom[adapter.namespace],
          10
        );
        if (
      ...
      if (
        ...
        obj.common.custom[adapter.namespace].ignoreAboveNumber =
          parseFloat(obj.common.custom[adapter.namespace].ignoreAboveNumber) ||
          null;
      }
      ...
      if (
        ...
        obj.common.custom[adapter.namespace].ignoreBelowNumber =
          parseFloat(obj.common.custom[adapter.namespace].ignoreBelowNumber) ||
          null;
      } else if (
      ...
      if (
        ...
        obj.common.custom[adapter.namespace].changesRelogInterval === 0
      ) {
        obj.common.custom[adapter.namespace].changesRelogInterval =
          parseInt(
            obj.common.custom[adapter.namespace].changesRelogInterval,
        ...
        obj.common.custom[adapter.namespace].changesRelogInterval =
          adapter.config.changesRelogInterval;
      }

      // changesMinDelta
      if (
        obj.common.custom[adapter.namespace].changesMinDelta ||
        obj.common.custom[adapter.namespace].changesMinDelta === 0
      ) {
        obj.common.custom[adapter.namespace].changesMinDelta =
          parseFloat(
            obj.common.custom[adapter.namespace].changesMinDelta
        ...
        obj.common.custom[adapter.namespace].changesMinDelta =
          adapter.config.changesMinDelta;
      }
      ...
      if (
        adapter._influxDPs[formerAliasId] &&
        !adapter._influxDPs[formerAliasId].storageTypeAdjustedInternally &&
        adapter._influxDPs[formerAliasId][adapter.namespace] &&
        isEqual(
          obj.common.custom[adapter.namespace],
          adapter._influxDPs[formerAliasId][adapter.namespace]
        )
      ...
      if (
        adapter._influxDPs[formerAliasId] &&
        adapter._influxDPs[formerAliasId].relogTimeout
      ) {
        clearTimeout(adapter._influxDPs[formerAliasId].relogTimeout);
        adapter._influxDPs[formerAliasId].relogTimeout = null;
      }

      const state = adapter._influxDPs[formerAliasId]
        ? adapter._influxDPs[formerAliasId].state
        : null;
      const skipped = adapter._influxDPs[formerAliasId]
        ? adapter._influxDPs[formerAliasId].skipped
        : null;
      const timeout = adapter._influxDPs[formerAliasId]
        ? adapter._influxDPs[formerAliasId].timeout
        : null;
      ...
      if (adapter._influxDPs[id] && adapter._influxDPs[id][adapter.namespace]) {
        adapter._influxDPs[id].relogTimeout &&
          clearTimeout(adapter._influxDPs[id].relogTimeout);
        adapter._influxDPs[id].timeout &&
...
function startPing(adapter) {
  adapter._pingInterval =
    adapter._pingInterval ||
    setInterval(() => ping(adapter), adapter.config.pingInterval);
}
...
function ping(adapter) {
  adapter._client.ping &&
    adapter.config.pingserver !== false &&
    adapter._client
      .ping(
        adapter.config.pingInterval - 1000 < 0
          ? 1000
          : adapter.config.pingInterval - 1000
      )
      .then(
        (hosts) => {
        ...
        (error) => {
          adapter.log.error(
            `Error during ping: ${error}. Attempting reconnect.`
          );
          reconnect(adapter);
        }
      );
...
function connect(adapter) {
  ...
  adapter.log.info(
    `Connecting ${adapter.config.protocol}://${adapter.config.host}:${
      adapter.config.port
    }/${
      adapter.config.dbversion !== "1.x" ? adapter.config.path || "" : ""
    } ...`
  );

  adapter.config.dbname = adapter.config.dbname || appName;
  adapter.config.validateSSL =
    adapter.config.validateSSL !== undefined
      ? !!adapter.config.validateSSL
      : true;

  adapter.config.seriesBufferMax =
    parseInt(adapter.config.seriesBufferMax, 10) || 0;
  ...
  switch (adapter.config.dbversion) {
    case "3.x":
      if (/[\x00-\x08\x0E-\x1F\x80-\xFF]/.test(adapter.config.token)) {
        adapter.log.error(
          "Token error: Please re-enter the token in Admin. Stopping"
        );
        return;
      }
      adapter._client = new DatabaseInfluxDB3x(
        adapter.log,
        adapter.config.host,
    ...
    if (err) {
      ...
      if (!dbNames.includes(adapter.config.dbname)) {
        adapter._client.createDatabase(adapter.config.dbname, (err) => {
          ...
          adapter._client.applyRetentionPolicyToDB(
            adapter.config.dbname,
            adapter.config.retention,
            (err) => {
              if (err) {
...
function checkMetaDataStorageType(adapter) {
  adapter._client.getMetaDataStorageType((error, storageType) => {
    if (error)
      ...
      adapter.log.debug(
        `Storage type for metadata found in DB: ${storageType}`
      );
      if (
        (storageType === "tags" && !adapter.config.usetags) ||
        (storageType === "fields" && adapter.config.usetags)
      ) {
        adapter.log.error(
          `Cannot use ${
            adapter.config.usetags ? "tags" : "fields"
          } for metadata (q, ack, from) since ` +
...
function processStartValues(adapter) {
  if (adapter._tasksStart && adapter._tasksStart.length) {
    ...
    if (
      adapter._influxDPs[taskId] &&
      adapter._influxDPs[taskId][adapter.namespace].changesOnly
    ) {
...
function testConnection(adapter, msg) {
  ...
  msg.message.config.requestTimeout =
    parseInt(msg.message.config.requestTimeout) || 30000;
          ...
          adapter.getForeignObject(
            ...
            (err, obj) => {
              if (!err) {
                ...
                adapter.log.error(
                  `Cannot read object "system.adapter.${adapter.namespace}": ${err}`
                );
                adapter.stop();
              }
            }
...
function processMessage(adapter, msg) {
  ...
  if (msg.command === "features") {
    ...
    adapter.config.dbversion !== "2.x"
      ? getHistory(adapter, msg)
      : getHistoryIflx2(adapter, msg);
...
function getConflictingPoints(adapter, msg) {
  return adapter.sendTo(
    msg.from,
    msg.command,
    { conflictingPoints: adapter._conflictingPoints },
    msg.callback
  );
}
...
function main(adapter) {
    ...
    if (obj && obj.common && !obj.common.defaultHistory) {
      ...
      adapter.setForeignObject("system.config", obj, (err) => {
        if (err) {
          ...
          adapter.log.info(
            `Set default history instance to "${adapter.namespace}"`
          );
        }
      });
    }
  ...
  adapter.config.reconnectInterval =
    parseInt(adapter.config.reconnectInterval, 10) || 10000;
  adapter.config.pingInterval =
    parseInt(adapter.config.pingInterval, 10) || 15000;

  if (
    adapter.config.round !== null &&
    adapter.config.round !== undefined &&
    adapter.config.round !== ""
  ) {
    adapter.config.round = parseInt(adapter.config.round, 10);
    if (!isFinite(adapter.config.round) || adapter.config.round < 0) {
      adapter.config.round = null;
      adapter.log.info(
        `Invalid round value: ${adapter.config.round} - ignore, do not round values`
      );
    } else {
  ...
  if (
    adapter.config.changesRelogInterval !== null &&
    adapter.config.changesRelogInterval !== undefined
  ) {
    adapter.config.changesRelogInterval = parseInt(
      adapter.config.changesRelogInterval,
      10
    );
  } else {
  ...
  adapter.config.seriesBufferFlushInterval =
    parseInt(adapter.config.seriesBufferFlushInterval, 10) || 600;

  adapter.config.requestTimeout =
    parseInt(adapter.config.requestTimeout, 10) || 30000;

  if (
    adapter.config.changesMinDelta !== null &&
    adapter.config.changesMinDelta !== undefined
  ) {
    adapter.config.changesMinDelta = parseFloat(
      adapter.config.changesMinDelta.toString().replace(/,/g, ".")
    );
  } else {
  ...
  if (
    adapter.config.blockTime !== null &&
    adapter.config.blockTime !== undefined
  ) {
    adapter.config.blockTime = parseInt(adapter.config.blockTime, 10) || 0;
  } else {
    if (
      adapter.config.debounce !== null &&
      adapter.config.debounce !== undefined
    ) {
  ...
  if (
    adapter.config.debounceTime !== null &&
    adapter.config.debounceTime !== undefined
  ) {
    adapter.config.debounceTime =
      parseInt(adapter.config.debounceTime, 10) || 0;
  } else {
  ...
  if (adapter.config.retention === -1) {
    // Custom timeframe
    adapter.config.retention =
      (parseInt(adapter.config.customRetentionDuration, 10) || 0) *
      24 *
      60 *
      60;
  }
  ...
  try {
    if (fs.statSync(cacheFile).isFile()) {
      ...
      if (tempData.seriesBufferCounter)
        adapter._seriesBufferCounter = tempData.seriesBufferCounter;
      if (tempData.seriesBuffer) adapter._seriesBuffer = tempData.seriesBuffer;
      if (tempData.conflictingPoints)
        adapter._conflictingPoints = tempData.conflictingPoints;
      adapter.log.info(
        `Buffer initialized with data for ${
          adapter._seriesBufferCounter
        } points and ${
          Object.keys(adapter._conflictingPoints).length
        } conflicts from last exit`
      );
    ...
    if (doc && doc.rows) {
      ...
      for (let i = 0; i < l; i++) {
        if (doc.rows[i].value) {
          ...
          const realId = id;
          if (
            doc.rows[i].value[adapter.namespace] &&
            doc.rows[i].value[adapter.namespace].aliasId
          ) {
            adapter._aliasMap[id] =
              doc.rows[i].value[adapter.namespace].aliasId;
            adapter.log.debug(
              `Found Alias: ${id} --> ${adapter._aliasMap[id]}`
            );
            id = adapter._aliasMap[id];
          }
          ...
          if (
            !adapter._influxDPs[id][adapter.namespace] ||
            typeof adapter._influxDPs[id][adapter.namespace] !== "object" ||
            adapter._influxDPs[id][adapter.namespace].enabled === false
          ) {
            delete adapter._influxDPs[id];
          } else {
            count++;
            adapter.log.info(
              `enabled logging of ${id}, Alias=${
                id !== realId
              }, ${count} points now activated`
            );

            // debounceTime and debounce compatibility handling
            if (
              !adapter._influxDPs[id][adapter.namespace].blockTime &&
              adapter._influxDPs[id][adapter.namespace].blockTime !== "0" &&
              adapter._influxDPs[id][adapter.namespace].blockTime !== 0
            ) {
              if (
                !adapter._influxDPs[id][adapter.namespace].debounce &&
                adapter._influxDPs[id][adapter.namespace].debounce !== "0" &&
                adapter._influxDPs[id][adapter.namespace].debounce !== 0
              ) {
                adapter._influxDPs[id][adapter.namespace].blockTime =
                  parseInt(adapter.config.blockTime, 10) || 0;
            ...
            if (
              !adapter._influxDPs[id][adapter.namespace].debounceTime &&
              adapter._influxDPs[id][adapter.namespace].debounceTime !== "0" &&
              adapter._influxDPs[id][adapter.namespace].debounceTime !== 0
            ) {
              adapter._influxDPs[id][adapter.namespace].debounceTime =
                parseInt(adapter.config.debounceTime, 10) || 0;
            ...
            adapter._influxDPs[id][adapter.namespace].changesOnly =
              adapter._influxDPs[id][adapter.namespace].changesOnly ===
                "true" ||
              adapter._influxDPs[id][adapter.namespace].changesOnly === true;
            adapter._influxDPs[id][adapter.namespace].ignoreZero =
              adapter._influxDPs[id][adapter.namespace].ignoreZero === "true" ||
              adapter._influxDPs[id][adapter.namespace].ignoreZero === true;

            // round
            if (
              adapter._influxDPs[id][adapter.namespace].round !== null &&
              adapter._influxDPs[id][adapter.namespace].round !== undefined &&
              adapter._influxDPs[id][adapter.namespace] !== ""
            ) {
              adapter._influxDPs[id][adapter.namespace].round = parseInt(
                adapter._influxDPs[id][adapter.namespace],
                10
              );
              if (
                !isFinite(adapter._influxDPs[id][adapter.namespace].round) ||
                adapter._influxDPs[id][adapter.namespace].round < 0
              ) {
                adapter._influxDPs[id][adapter.namespace].round =
                  adapter.config.round;
              } else {
                adapter._influxDPs[id][adapter.namespace].round = Math.pow(
                  10,
                  parseInt(adapter._influxDPs[id][adapter.namespace].round, 10)
                );
              }
            } else {
              adapter._influxDPs[id][adapter.namespace].round =
            ...
            // ignoreAboveNumber
            if (
              adapter._influxDPs[id][adapter.namespace].ignoreAboveNumber !==
                undefined &&
              adapter._influxDPs[id][adapter.namespace].ignoreAboveNumber !==
                null &&
              adapter._influxDPs[id][adapter.namespace].ignoreAboveNumber !== ""
            ...
            if (
              adapter._influxDPs[id][adapter.namespace].ignoreBelowNumber !==
                undefined &&
              adapter._influxDPs[id][adapter.namespace].ignoreBelowNumber !==
                null &&
              adapter._influxDPs[id][adapter.namespace].ignoreBelowNumber !== ""
              ...
              adapter._influxDPs[id][adapter.namespace].ignoreBelowZero ===
                "true" ||
              adapter._influxDPs[id][adapter.namespace].ignoreBelowZero === true
            ) {
            ...
            if (
              adapter._influxDPs[id][adapter.namespace]
                .disableSkippedValueLogging !== undefined &&
              adapter._influxDPs[id][adapter.namespace]
                .disableSkippedValueLogging !== null &&
              adapter._influxDPs[id][adapter.namespace]
                .disableSkippedValueLogging !== ""
            ) {
              adapter._influxDPs[id][
                adapter.namespace
              ].disableSkippedValueLogging =
                adapter._influxDPs[id][adapter.namespace]
            ...
            // enableDebugLogs
            if (
              adapter._influxDPs[id][adapter.namespace].enableDebugLogs !==
                undefined &&
              adapter._influxDPs[id][adapter.namespace].enableDebugLogs !==
                null &&
              adapter._influxDPs[id][adapter.namespace].enableDebugLogs !== ""
            ...
            if (
              adapter._influxDPs[id][adapter.namespace].changesRelogInterval !==
                undefined &&
              adapter._influxDPs[id][adapter.namespace].changesRelogInterval !==
                null &&
              adapter._influxDPs[id][adapter.namespace].changesRelogInterval !==
            ...
            if (
              adapter._influxDPs[id][adapter.namespace].changesMinDelta !==
                undefined &&
              adapter._influxDPs[id][adapter.namespace].changesMinDelta !==
                null &&
              adapter._influxDPs[id][adapter.namespace].changesMinDelta !== ""
            ...
            adapter._influxDPs[id][adapter.namespace].storageType =
              adapter._influxDPs[id][adapter.namespace].storageType || false;
  ...
  if (adapter._influxDPs[id]) {
    ...
    if (!timerRelog) {
      ...
      if (adapter._influxDPs[id].timeout && state.ts !== state.lc) {
        ...
        return;
      } else if (adapter._influxDPs[id].timeout) {
      ...
      if (
        !valueUnstable &&
        settings.blockTime &&
        adapter._influxDPs[id].state &&
        adapter._influxDPs[id].state.ts + settings.blockTime > state.ts
      ) {
        settings.enableDebugLogs &&
          adapter.log.debug(
            `value ignored blockTime ${id}, value=${state.val}, ts=${state.ts}, lastState.ts=${adapter._influxDPs[id].state.ts}, blockTime=${settings.blockTime}`
          );
        return;
      }
      ...
      if (
        settings.ignoreZero &&
        (state.val === undefined || state.val === null || state.val === 0)
      ) {
        settings.enableDebugLogs &&
          adapter.log.debug(
      ...
      if (adapter._influxDPs[id].state && settings.changesOnly) {
        if (settings.changesRelogInterval === 0) {
          if (
            (adapter._influxDPs[id].state.val !== null || state.val === null) &&
            state.ts !== state.lc
          ) {
            ...
            settings.enableDebugLogs &&
              adapter.log.debug(
                `value not changed ${id}, last-value=${adapter._influxDPs[id].state.val}, new-value=${state.val}, ts=${state.ts}`
              );
            return;
          }
          ...
          if (
            (adapter._influxDPs[id].state.val !== null || state.val === null) &&
            state.ts !== state.lc &&
            Math.abs(adapter._influxDPs[id].lastLogTime - state.ts) <
          ...
          if (state.ts !== state.lc) {
            settings.enableDebugLogs &&
              adapter.log.debug(
                `value-not-changed-relog ${id}, value=${state.val}, lastLogTime=${adapter._influxDPs[id].lastLogTime}, ts=${state.ts}`
              );
            ignoreDebonce = true;
          }
        }
        if (typeof state.val === "number") {
          if (
            ...
            Math.abs(adapter._influxDPs[id].state.val - state.val) <
            ...
            settings.enableDebugLogs &&
              adapter.log.debug(
                `Min-Delta not reached ${id}, last-value=${adapter._influxDPs[id].state.val}, new-value=${state.val}, ts=${state.ts}`
              );
            return;
          } else if (settings.changesMinDelta !== 0) {
            settings.enableDebugLogs &&
              adapter.log.debug(
                `Min-Delta reached ${id}, last-value=${adapter._influxDPs[id].state.val}, new-value=${state.val}, ts=${state.ts}`
              );
          }
        } else {
          settings.enableDebugLogs &&
            adapter.log.debug(
              `Min-Delta ignored because no number ${id}, last-value=${adapter._influxDPs[id].state.val}, new-value=${state.val}, ts=${state.ts}`
            );
        }
      }
    }
    ...
    if (timerRelog) {
      ...
      settings.enableDebugLogs &&
        adapter.log.debug(
          `timed-relog ${id}, value=${state.val}, lastLogTime=${adapter._influxDPs[id].lastLogTime}, ts=${state.ts}`
        );
      ignoreDebonce = true;
    } else {
      if (settings.changesOnly && adapter._influxDPs[id].skipped) {
        settings.enableDebugLogs &&
          adapter.log.debug(
            `Skipped value logged ${id}, value=${adapter._influxDPs[id].skipped.val}, ts=${adapter._influxDPs[id].skipped.ts}`
          );
    ...
    if (settings.debounceTime && !ignoreDebonce && !timerRelog) {
      ...
      adapter._influxDPs[id].timeout &&
        clearTimeout(adapter._influxDPs[id].timeout);
      adapter._influxDPs[id].timeout = setTimeout(
        (id, state) => {
          adapter._influxDPs[id].timeout = null;
          adapter._influxDPs[id].state = state;
          ...
          settings.enableDebugLogs &&
            adapter.log.debug(
              `Value logged ${id}, value=${adapter._influxDPs[id].state.val}, ts=${adapter._influxDPs[id].state.ts}`
            );
          pushHelper(adapter, id);
          if (settings.changesOnly && settings.changesRelogInterval > 0) {
            adapter._influxDPs[id].relogTimeout = setTimeout(
              reLogHelper,
              settings.changesRelogInterval * 1000,
              adapter,
              id
...
function reLogHelper(adapter, _id) {
  ...
  if (adapter._influxDPs[_id].skipped) {
    ...
    adapter.getForeignState(adapter._influxDPs[_id].realId, (err, state) => {
      if (err) {
        adapter.log.info(
          `init timed Relog: can not get State for ${_id}: ${err}`
        );
      } else if (!state) {
        adapter.log.info(
          `init timed Relog: disable relog because state not set so far for ${_id}: ${JSON.stringify(
            state
          )}`
        );
...
function pushHelper(adapter, _id, state, cb) {
  ...
  const _settings =
    (adapter._influxDPs[_id] && adapter._influxDPs[_id][adapter.namespace]) ||
    {};
  ...
  if (typeof state.val === "number" && !isFinite(state.val)) {
    ...
    return (
      ...
      setImmediate(
        cb,
        `Non Finite value ${state.val} for ${_id} can not be handled`
      )
    );
  }
  ...
  if (
    state.val !== null &&
    (typeof state.val === "object" || typeof state.val === "undefined")
  ) {
  ...
  _settings.enableDebugLogs &&
    adapter.log.debug(
      `Datatype ${_id}: Currently: ${typeof state.val}, StorageType: ${
  ...
  if (_settings.storageType === "String" && typeof state.val !== "string") {
    ...
    if (typeof state.val === "boolean") {
      ...
      adapter.log.info(
        `Do not store value "${state.val}" for ${_id} because no number`
      );
      return (
        cb &&
        setImmediate(cb, `do not store value for ${_id} because no number`)
      );
    }
  } else if (
    _settings.storageType === "Boolean" &&
...
function pushValueIntoDB(adapter, id, state, directWrite, cb) {
  ...
  if (typeof state.val === "number" && !isFinite(state.val)) {
    return (
      cb && cb(`InfluxDB can not handle non finite values like ${state.val}`)
    );
  }
  ...
  if (
    (adapter._conflictingPoints[id] ||
      adapter.config.seriesBufferMax === 0 ||
      directWrite) &&
    adapter._connected &&
    adapter._client.request &&
    adapter._client.request.getHostsAvailable().length > 0
  ) {
    if (adapter.config.seriesBufferMax !== 0) {
      adapter.log.debug(
        `Direct writePoint("${id} - ${influxFields.value} / ${influxFields.time}")`
      );
    }
...
function addPointToSeriesBuffer(adapter, id, stateObj, cb) {
  if (
    (adapter._conflictingPoints[id] || adapter.config.seriesBufferMax === 0) &&
    adapter._connected &&
    adapter._client.request &&
    adapter._client.request.getHostsAvailable().length > 0
  ) {
    if (adapter.config.seriesBufferMax !== 0) {
      adapter.log.debug(
        `Direct writePoint("${id} - ${stateObj.value} / ${stateObj.time}")`
      );
    }
  ...
  adapter._seriesBufferCounter++;
  if (
    adapter._seriesBufferCounter > adapter.config.seriesBufferMax &&
    adapter._connected &&
    adapter._client.request &&
    adapter._client.request.getHostsAvailable().length > 0 &&
    !adapter._seriesBufferFlushPlanned
  ) {
    // flush out
    adapter._seriesBufferFlushPlanned = true;
...
function storeBufferedSeries(adapter, id, cb) {
  ...
  if (id) {
    ...
    adapter._seriesBuffer[id] = [];
    adapter.log.debug(
      `Store ${idSeries.length} buffered influxDB history points for ${id}`
    );
  ...
  adapter.log.info(
    `Store ${adapter._seriesBufferCounter} buffered influxDB history points`
  );
  ...
  if (adapter._seriesBufferCounter > 15000) {
    ...
    adapter.log.info(
      `Too many data points (${adapter._seriesBufferCounter}) to write at once; write per ID`
    );
    writeAllSeriesPerID(
      adapter,
      currentBuffer,
      (err) => cb && cb(err, adapter._seriesBufferCounter)
    );
  } else {
    writeAllSeriesAtOnce(
      adapter,
      currentBuffer,
      (err) => cb && cb(err, adapter._seriesBufferCounter)
    );
  ...
  adapter._client.writeSeries(series, (err /* , result */) => {
    if (err) {
      ...
      if (adapter._client.request.getHostsAvailable().length === 0) {
        ...
        adapter.log.warn(
          "All possible data points were written, others can not really be corrected"
        );
      } else {
        adapter.log.info(
          `Try to write ${
            Object.keys(series).length
          } Points separate to find the conflicting id`
        );
...
function writeSeriesPerID(adapter, seriesId, points, cb) {
  ...
  if (points.length) {
    ...
    adapter.log.info(
      `Too many dataPoints (${
        pointsToSend.length + points.length
      }) for "${seriesId}" to write at once; split in 15.000 batches`
    );
  }
  adapter._client.writePoints(seriesId, pointsToSend, (err) => {
    if (err) {
      ...
      if (
        adapter._client.request.getHostsAvailable().length === 0 ||
        (err.message &&
          (err.message === "timeout" || err.message.includes("timed out")))
      ) {
        adapter.log.info(
          "Host not available, move all points back in the Buffer"
        );
        ...
        adapter.log.warn(
          `Try to write ${pointsToSend.length} Points separate to find the conflicting one`
        );
        // we found the conflicting id
        return writePointsForID(adapter, seriesId, pointsToSend, () =>
...
function writePointsForID(adapter, seriesId, points, cb) {
  ...
  writeOnePointForID(adapter, seriesId, point[0], false, () =>
    setImmediate(() => writePointsForID(adapter, seriesId, points, cb))
  );
}
...
function writeOnePointForID(adapter, pointId, point, directWrite, cb) {
  ...
  adapter._client.writePoint(pointId, point, null, (err /* , result */) => {
    if (err) {
      adapter.log.warn(
        `Error on writePoint("${JSON.stringify(
          point
        )}): ${err} / ${JSON.stringify(err.message)}"`
      );
      if (
        !adapter._client.request.getHostsAvailable().length ||
        (err.message && err.message === "timeout")
      ) {
        reconnect(adapter);
        addPointToSeriesBuffer(adapter, pointId, point);
      } else if (
        ...
        if (
          adapter._influxDPs[pointId] &&
          adapter._influxDPs[pointId][adapter.namespace] &&
          !adapter._influxDPs[pointId][adapter.namespace].storageType
        ) {
          let convertDirection = "";
          if (
            err.message.includes(
              "is type bool, already exists as type float"
            ) ||
            err.message.includes(
              "is type boolean, already exists as type float"
            )
          ) {
            ...
            err.message.includes(
              "is type float, already exists as type bool"
            ) ||
            ...
            err.message.includes("is type string, already exists as type float")
          ) {
          ...
          if (retry) {
            adapter.log.info(
              `Try to convert ${convertDirection} and re-write for ${pointId} and set storageType to ${
                adapter._influxDPs[pointId][adapter.namespace].storageType
              }`
            );
          ...
          if (adjustType) {
            ...
            obj.common.custom = {};
            obj.common.custom[adapter.namespace] = {};
            obj.common.custom[adapter.namespace].storageType =
              adapter._influxDPs[pointId][adapter.namespace].storageType;
            adapter.extendForeignObject(pointId, obj, (err) => {
              if (err) {
                adapter.log.error(
                  `error updating history config for ${pointId} to pin datatype: ${err}`
                );
              } else {
                adapter.log.info(
                  `changed history configuration to pin detected datatype for ${pointId}`
                );
              }
            });
          }
        ...
        if (!directWrite || !retry) {
          ...
          adapter.log.warn(
            `Add ${pointId} to conflicting Points (${
              Object.keys(adapter._conflictingPoints).length
            } now)`
          );
        }
        ...
        if (adapter._errorPoints[pointId] < 10) {
          ...
          adapter.log.info(
            `Add point that had error for ${pointId} to buffer again, error-count=${adapter._errorPoints[pointId]}`
          );
          addPointToSeriesBuffer(adapter, pointId, point);
        } else {
          adapter.log.info(
            `Discard point that had error for ${pointId}, error-count=${adapter._errorPoints[pointId]}`
          );
...
function writeFileBufferToDisk() {
  ...
  if (adapter._seriesBufferCounter) {
    ...
    try {
      ...
      adapter.log.warn(
        `Store data for ${fileData.seriesBufferCounter} points and ${
          Object.keys(fileData.conflictingPoints).length
        } conflicts`
      );
    } catch (err) {
...
function _delete(adapter, id, state, cb) {
  ...
  if (adapter.config.dbversion === "1.x" || dapter.config.dbversion === "3.x") {
    let query;
    if (state.ts) {
      query = `DELETE FROM "${id}" WHERE time = '${new Date(
        state.ts
      ).toISOString()}'`;
    } else if (state.start) {
      query = `DELETE FROM "${id}" WHERE time >= '${new Date(
    ...
    adapter._client.query(query, (err) => {
      if (err) {
        adapter.log.warn(
          `Error on delete("${query}): ${err} / ${JSON.stringify(err.message)}"`
        );
        cb && cb(err);
      } else {
        setConnected(adapter, true);
    ...
    adapter._client.deleteData(
      ...
      (err) => {
        if (err) {
          adapter.log.warn(
            `Error on delete("${err} / ${JSON.stringify(err.message)}"`
          );
          cb && cb(err);
        } else {
          setConnected(adapter, true);
...
function deleteState(adapter, msg) {
  ...
  if (Array.isArray(msg.message)) {
    ...
    for (let i = 0; i < msg.message.length; i++) {
      id = adapter._aliasMap[msg.message[i].id]
        ? adapter._aliasMap[msg.message[i].id]
        : msg.message[i].id;
      ...
      if (msg.message[i].ts) {
        ...
        _delete(adapter, id, {
          start: msg.message[i].start,
          end: msg.message[i].end || Date.now(),
        });
      } else if (
        ...
        if (typeof msg.message[i].state.start === "string") {
          msg.message[i].state.start = new Date(
            msg.message[i].state.start
          ).getTime();
        }
        if (typeof msg.message[i].state.end === "string") {
          msg.message[i].state.end = new Date(
            msg.message[i].state.end
          ).getTime();
        }
        _delete(adapter, id, {
    ...
    id = adapter._aliasMap[msg.message.id]
      ? adapter._aliasMap[msg.message.id]
      : msg.message.id;
      ...
      if (msg.message.state[j] && typeof msg.message.state[j] === "object") {
        if (msg.message.state[j].ts) {
          ...
          if (typeof msg.message.state[j].start === "string") {
            msg.message.state[j].start = new Date(
              msg.message.state[j].start
            ).getTime();
          }
          if (typeof msg.message.state[j].end === "string") {
            msg.message.state[j].end = new Date(
              msg.message.state[j].end
            ).getTime();
          }
          _delete(adapter, id, {
        ...
        msg.message.state[j] &&
        typeof msg.message.state[j] === "number"
      ) {
        _delete(adapter, id, { ts: msg.message.state[j] });
      } else {
        adapter.log.warn(
          `Invalid state for ${JSON.stringify(msg.message.state[j])}`
        );
      }
    }
    ...
    for (let j = 0; j < msg.message.ts.length; j++) {
      if (msg.message.ts[j] && typeof msg.message.ts[j] === "number") {
        ...
        adapter.log.warn(
          `Invalid state for ${JSON.stringify(msg.message.ts[j])}`
        );
      }
    }
  } else if (
    msg.message.id &&
    msg.message.state &&
    typeof msg.message.state === "object"
  ) {
    adapter.log.debug("deleteState 1 item");
    id = adapter._aliasMap[msg.message.id]
      ? adapter._aliasMap[msg.message.id]
      : msg.message.id;
    return _delete(adapter, id, { ts: msg.message.state.ts }, (error) =>
      adapter.sendTo(
        msg.from,
        msg.command,
        {
    ...
    msg.message.id &&
    msg.message.ts &&
    typeof msg.message.ts === "number"
  ) {
    adapter.log.debug("deleteState 1 item");
    id = adapter._aliasMap[msg.message.id]
      ? adapter._aliasMap[msg.message.id]
      : msg.message.id;
    return _delete(adapter, id, { ts: msg.message.ts }, (error) =>
      adapter.sendTo(
        msg.from,
        msg.command,
        {
...
function update(adapter, id, state, cb) {
  ...
  if (
    adapter.config.dbversion === "1.x" ||
    adapter.config.dbversion === "3.x"
  ) {
    let query = `SELECT * FROM "${id}" WHERE time = '${new Date(
      state.ts
    ).toISOString()}'`;
    ...
    adapter._client.query(query, (err, result) => {
      if (err) {
        adapter.log.warn(
          `Error on update("${query}): ${err} / ${JSON.stringify(err.message)}"`
        );
        cb && cb(err);
      } else {
        setConnected(adapter, true);
        ...
        if (result[0] && result[0][0]) {
          ...
          _delete(adapter, id, { ts: stored.ts }, (error) => {
            if (error) {
              adapter.log.error(
                `Cannot delete value for ${id}: ${JSON.stringify(state)}`
              );
              cb && cb(err);
            } else {
          ...
          adapter.log.error(
            `Cannot find value to delete for ${id}: ${JSON.stringify(state)}`
          );
          cb && cb("not found");
        }
      }
    ...
    fluxQuery += ` |> range(start:time(v:${state.ts * 1000000}), stop:time(v:${
      state.ts * 1000000 + 1000
    }))`;
...
function updateState(adapter, msg) {
  ...
  if (Array.isArray(msg.message)) {
    ...
    id = adapter._aliasMap[msg.message.id]
      ? adapter._aliasMap[msg.message.id]
      : msg.message.id;
    update(adapter, id, msg.message.state, (error) =>
      adapter.sendTo(
        msg.from,
        msg.command,
        {
...
function storeStatePushData(adapter, id, state, applyRules) {
  ...
  if (!adapter._influxDPs[id] || !adapter._influxDPs[id][adapter.namespace]) {
    if (applyRules) {
      throw new Error(
        `influxdb not enabled for ${id}, so can not apply the rules as requested`
      );
    }
...
async function storeState(adapter, msg) {
  ...
  if (Array.isArray(msg.message)) {
    ...
    for (let i = 0; i < msg.message.length; i++) {
      const id = adapter._aliasMap[msg.message[i].id]
        ? adapter._aliasMap[msg.message[i].id]
        : msg.message[i].id;
      try {
        await storeStatePushData(
          adapter,
          id,
          msg.message[i].state,
          msg.message.rules
    ...
    const id = adapter._aliasMap[msg.message.id]
      ? adapter._aliasMap[msg.message.id]
      : msg.message.id;
    for (let j = 0; j < msg.message.state.length; j++) {
      try {
  ...
  if (errors.length) {
    adapter.log.warn(
      `storeState executed with ${errors.length} errors: ${errors.join(", ")}`
    );
    return adapter.sendTo(
      msg.from,
      msg.command,
  ...
  adapter.log.debug(
    `storeState executed with ${successCount} states successfully`
  );
  adapter.sendTo(
    msg.from,
    msg.command,
...
*/
    if (
      adapter._influxDPs[id].skipped &&
      !(
        adapter._influxDPs[id][adapter.namespace] &&
        adapter._influxDPs[id][adapter.namespace].disableSkippedValueLogging
      )
    ) {
      count++;
      pushHelper(adapter, id, adapter._influxDPs[id].skipped, () => {
...
function getHistory(adapter, msg) {
  ...
  const options = {
    ...
    logId:
      (msg.message.id ? msg.message.id : "all") + Date.now() + Math.random(),
  };

  adapter.log.debug(
    `${options.logId} getHistory message: ${JSON.stringify(msg.message)}`
  );
  ...
  try {
    ...
    return adapter.sendTo(
      ...
      {
        error:
          "Invalid call. Start date " +
          JSON.stringify(options.start) +
          " is not a valid date",
      },
      msg.callback
    );
  ...
  try {
    ...
    return adapter.sendTo(
      ...
      {
        error:
          "Invalid call. End date " +
          JSON.stringify(options.end) +
          " is not a valid date",
      },
      msg.callback
    );
  ...
  const debugLog = (options.debugLog = !!(
    adapter._influxDPs[options.id] &&
    adapter._influxDPs[options.id][adapter.namespace] &&
    adapter._influxDPs[options.id][adapter.namespace].enableDebugLogs
  ));
  ...
  if (
    (options.aggregate === "percentile" && options.percentile < 0) ||
    options.percentile > 100
  ) {
    adapter.log.error(
      `Invalid percentile value: ${options.percentile}, use 50 as default`
    );
    options.percentile = 50;
  }

  if (
    (options.aggregate === "quantile" && options.quantile < 0) ||
    options.quantile > 1
  ) {
    adapter.log.error(
      `Invalid quantile value: ${options.quantile}, use 0.5 as default`
    );
  ...
  if (
    ...
    adapter.log.error(
      `Invalid integralUnit value: ${options.integralUnit}, use 60s as default`
    );
  ...
  debugLog &&
    adapter.log.debug(
      `${options.logId} getHistory (InfluxDB1) call: ${JSON.stringify(options)}`
    );
  ...
  if (
    options.aggregate === "integral" &&
    options.integralInterpolation === "linear"
  ) {
  ...
  if (!options.removeBorderValues) {
    let addQuery = "";
    if (options.start) {
      addQuery = `SELECT value, time from "${options.id}" WHERE time <= '${new Date(
        options.start
      ).toISOString()}' ORDER BY time DESC LIMIT 1;`;
  ...
  debugLog &&
    adapter.log.debug(`${options.logId} History-Queries to execute: ${query}`);

  storeBufferedSeries(adapter, options.id, (err, storedCount) => {
    if (err) {
      adapter.log.info(
        `Error storing buffered series for ${options.id} before GetHistory: ${err}`
      );
    }
    setTimeout(
      () => {
        if (!adapter._client) {
          return Aggregate.sendResponse(
            adapter,
            msg,
            options,
        ...
        adapter._client.query(query, (error, rows) => {
          ...
          debugLog &&
            adapter.log.debug(
              `${options.logId} Response rows: ${JSON.stringify(rows)}`
            );
          ...
          if (rows && rows.length) {
            for (let qr = 0; qr < rows.length; qr++) {
              for (let rr = 0; rr < rows[qr].length; rr++) {
                if (
                  rows[qr][rr].val === undefined &&
                  rows[qr][rr].value !== undefined
                ) {
                ...
                if (typeof rows[qr][rr].val === "number" && options.round) {
                  rows[qr][rr].val =
                    Math.round(rows[qr][rr].val * options.round) /
                    options.round;
                }
          ...
          try {
            Aggregate.sendResponse(
              ...
              msg,
              options,
              (error ? error.toString() : null) || result,
              startTime
            );
          } catch (e) {
            Aggregate.sendResponse(
...
function getHistoryIflx2(adapter, msg) {
  ...
  debugLog &&
    adapter.log.debug(
      `${options.logId} getHistory (InfluxDB2) options final: ${JSON.stringify(
        options
      )}`
    );
  ...
  const booleanTypeCheckQuery = `
        ...
        |> range(${
          options.start
            ...
            : `start: ${new Date(
                options.end - (adapter.config.retention || 31536000) * 1000
              ).toISOString()}, `
        }stop: ${new Date(options.end).toISOString()})
        |> filter(fn: (r) => r["_field"] == "value")
        |> filter(fn: (r) => r["_measurement"] == "${
          options.id
        }" and contains(value: r._value, set: [true, false]))
        ${
          adapter.config.usetags
        ...
        adapter._client &&
          adapter._client.query(booleanTypeCheckQuery, (error, rslt) => {
            ...
            if (
              adapter._influxDPs[options.id] &&
              adapter._influxDPs[options.id][adapter.namespace] &&
              adapter._influxDPs[options.id][adapter.namespace].storageType &&
              adapter._influxDPs[options.id][adapter.namespace].storageType !==
                "Number"
            ) {
              ...
              if (error.message.includes("type conflict: bool")) {
                debugLog &&
                  adapter.log.debug(
                    `${options.logId} Bool check error: ${error.message}`
                  );
              ...
              debugLog &&
                adapter.log.debug(
                  `${options.logId} Bool check result: ${JSON.stringify(rslt)}`
                );
              if (
                rslt.find(
                  (r) => r.error && r.error.includes("type conflict: bool")
                )
              ) {
                supportsAggregates = true;
              } else {
            ...
            if (supportsAggregates) {
              if (
                adapter._influxDPs[options.id] &&
                adapter._influxDPs[options.id][adapter.namespace] &&
                adapter._influxDPs[options.id][adapter.namespace].state &&
                typeof adapter._influxDPs[options.id][adapter.namespace].state
                  .val !== "number"
              ) {
                supportsAggregates = false;
              } else if (
                adapter._influxDPs[options.id] &&
                adapter._influxDPs[options.id][adapter.namespace] &&
                adapter._influxDPs[options.id][adapter.namespace].skipped &&
                typeof adapter._influxDPs[options.id][adapter.namespace].skipped
                  .val !== "number"
              ) {
                supportsAggregates = false;
              }
            ...
            if (!supportsAggregates) {
              debugLog &&
                adapter.log.debug(
                  `${options.logId} Measurement ${options.id} seems to be no number - skipping aggregation options`
                );
            }
            ...
            if (
              !(resultsFromInfluxDB && supportsAggregates) &&
              options.aggregate !== "minmax"
            ) {
            ...
            if (resultsFromInfluxDB && options.step && supportsAggregates) {
              ...
              switch (options.aggregate) {
                ...
                case "percentile":
                  fluxQuery += ` |> quantile(column: "${valueColumn}", q: ${
                    options.percentile / 100
                  })`;
                  break;
                ...
                case "integral":
                  fluxQuery += ` |> integral(column: "${valueColumn}", unit: ${
                    options.integralUnit
                  }s, interpolate: ${
                    options.integralInterpolation === "linear"
                      ? '"linear"'
                      : '""'
              ...
              if (options.start) {
                ...
                addFluxQuery = `from(bucket: "${adapter.config.dbname}") 
                        |> range(start: ${new Date(
                          options.start -
                            (adapter.config.retention || 31536000) * 1000
            ...
            debugLog &&
              adapter.log.debug(
                `${options.logId} History-queries to execute: ${fluxQueries}`
              );
            ...
            adapter._client &&
              adapter._client.queries(fluxQueries, (err, rows) => {
                ...
                debugLog &&
                  adapter.log.debug(
                    `${options.logId} Parsing retrieved rows:${JSON.stringify(
                      rows
                    )}`
                  );
                  ...
                  for (let qr = 0; qr < rows.length; qr++) {
                    for (let rr = 0; rr < rows[qr].length; rr++) {
                      ...
                      if (rows[qr][rr].time) {
                        ...
                        rows[qr][rr].ts = Math.round(
                          startTime + (stopTime - startTime) / 2
                        );
                      ...
                      if (options.addId) {
                        rows[qr][rr].id =
                          rows[qr][rr]._measurement || options.id;
                      }
...
function getEnabledDPs(adapter, msg) {
  ...
  for (const id in adapter._influxDPs) {
    if (
      adapter._influxDPs.hasOwnProperty(id) &&
      adapter._influxDPs[id] &&
      adapter._influxDPs[id][adapter.namespace] &&
      adapter._influxDPs[id][adapter.namespace].enabled
    ) {
      data[adapter._influxDPs[id].realId] =
        adapter._influxDPs[id][adapter.namespace];
    }
  }
